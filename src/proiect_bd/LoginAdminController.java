...
public class LoginAdminController {
	...
	LoginAdminController(Model model, LoginAdminView view) {
		this.model = model;
		this.view = view;
	...
	class LoginListener implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			try {
			    ...
			    model.cStmt = model.con.prepareCall("{? = call verifyAdminLogin(?, ?)}");
				...
				if(model.loginResult == 1)
					...
					AdminMainMenuView wp = new AdminMainMenuView(view.tf1.getText());
					AdminMainMenuController wpc = new AdminMainMenuController(model, wp);
				}
